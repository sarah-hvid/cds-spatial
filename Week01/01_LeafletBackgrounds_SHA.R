...
leaflet() %>%
  ...
  addProviderTiles("Esri.WorldImagery", 
                   options = providerTileOptions(opacity=0.5)) %>% 
  setView(lng = 176.005006, lat = -40.9767231, zoom = 6)
...
# inserted three locations from Als
...
DKmap %>% 
  addMarkers(lng = places$Longitude, 
             lat = places$Latitude,
             popup = paste(
               "<strong>Placename: </strong>",places$Placename,
               "<br> Type:",places$Type,
               "<br><strong>Description: </strong>",places$Description,
               "<br><strong>Stars: </strong>",places$Stars1_5,
