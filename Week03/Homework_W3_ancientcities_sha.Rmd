...
author: "Sarah Hvid"
date: "18/02/2022 updated `r Sys.Date()`"
output:
...
- When using the st_crs function, it can be read that the lengthunit is metres. 
...
tm_shape(ten_buffers)  +
  ...
  tm_shape(ten_cities) +
  tm_text("Ancient Toponym", auto.placement = 5) +
  tm_dots(col = 'red', 
               size = 0.1) +
...
# Libraries
pacman::p_load(tidytext, jsonlite, tidyverse)
...
unlist(inscriptions$coordinates)
...
i_sm <- inscriptions %>% 
  ...
  filter(!is.na(latitude))  %>%  # adding a filter removing the NA coordinates from the data
...
- The inscriptions are generally from south Europe, and the insciptions of the smaller subset are from Italy. 
...
cities_it <- st_intersection(insc_ch, select(cities_5km))
...
insc_urban <- st_intersection(insc_sf3035, c_buff) #10462 obs
insc_rural <- st_difference(insc_sf3035, c_buff) #1604 obs
