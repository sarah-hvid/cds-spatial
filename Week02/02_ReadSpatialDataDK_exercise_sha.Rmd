...
- playgrounds - 51 features, 5 fields, points
- forests - 32 features, 15 fields, and multi polygon
- parks - 47 features, 10 fields, multipolygon
- shelters - 19 features and 6 fields
...
plot(forests$geometry, col = "green");plot(parks$geometry, col = "pink", add = TRUE)
...
plotRGB(aarhus);plot(elevation, add = TRUE) # doesn't work
...
st_crs(playgrounds_crs) #+proj=utm +zone=32 +ellps=GRS80 +units=m +no_defs 
st_crs(elevation_crs) #+proj=utm +zone=32 +ellps=GRS80 +units=m +no_defs 
...
plotRGB(aarhus, stretch = "linear", alpha = 200);plot(playgrounds_crs, col = 'green', lwd = 5, add = TRUE)
  # stretch doesn't work, not sure how to fix it
...
plot(DK_border, col = 'red')
plot(municipalities, add = TRUE)
```
